...
$num = $info["number4"];
bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "• Running Now Checker : ❲ 4 ❳ ٫،!",]);
...
bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "- New UserName : @$user\n- loops online : $x\n-------------------------\n• Channel : @oBooo ."]);
                        ...
                        if($e->getMessage() == "The provided username is not valid"){
                            $data = str_replace("\n".$user,"", file_get_contents("u4"));
                            file_put_contents("u4", $data);
                            bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "4 • This username is (banned/smaller than 5) i delete it from u4 list : @$user ",]);
                        }elseif(preg_match('/FLOOD_WAIT_(.*)/i', $e->getMessage())){
                            $seconds = str_replace('FLOOD_WAIT_', '', $e->getMessage());
                            bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "4 • I m sleeping : $seconds ",]);
                            sleep($seconds);
                        }elseif($e->getMessage() == "USERNAME_OCCUPIED"){
                            $data = str_replace("\n".$user,"", file_get_contents("u4"));
                            file_put_contents("u4", $data);
                            bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "4 • could not save it : @$user",]);
