...
$num = $info["number5"];
bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "• Running Now Checker : ❲ 5 ❳ ٫،!",]);
...
bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "- New UserName : @$user\n- loops online : $x\n-------------------------\n• Channel : @oBooo ."]);
                        ...
                        if($e->getMessage() == "The provided username is not valid"){
                            $data = str_replace("\n".$user,"", file_get_contents("u5"));
                            file_put_contents("u5", $data);
                            bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "5 • This username is (banned/smaller than 5) i delete it from u5 list : @$user ",]);
                        }elseif(preg_match('/FLOOD_WAIT_(.*)/i', $e->getMessage())){
                            $seconds = str_replace('FLOOD_WAIT_', '', $e->getMessage());
                            bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "5 • I m sleeping : $seconds ",]);
                            sleep($seconds);
                        }elseif($e->getMessage() == "USERNAME_OCCUPIED"){
                            $data = str_replace("\n".$user,"", file_get_contents("u5"));
                            file_put_contents("u5", $data);
                            bot('sendMessage', ['chat_id' => file_get_contents("ID"), 'text' => "5 • could not save it : @$user",]);
